...
export const SearchResults = (props: ISearchProps): JSX.Element | null => {
  ...
  const relatedMatches: IProjectSearch[] = relatedQueryMatches(
    props.projects,
    props.query
  );
  ...
  const searchResults: IProjectSearch[] = matches.filter(
    (match) => match.query === props.query
  );
  const relatedResults: IProjectSearch[] = matches.filter(
    (match) => match.query !== props.query
  );
  const relatedResultTags =
    relatedResults &&
    relatedResults
      .map((project) => project.query)
      ...
      project.content.find(
        (section) =>
          ...
          section.highlight.find(
            (item) =>
              item.tags &&
              item.tags.find((tag) => tag.toLowerCase() === query.toLowerCase())
          )
      )
  );
  const titleMatches: IProject[] = projects.filter(
    (project: IProject) =>
      project.details.header.toLowerCase().indexOf(query.toLowerCase()) !== -1
  );

  const matches: IProject[] = removeDuplicates([
    ...tagMatches,
    ...skillMatches,
    ...titleMatches,
  ]);
...
const removeDuplicates = (matches: IProject[]): IProject[] =>
...
const removeDuplicateTitles = (matches: IProjectSearch[]) =>
  matches.filter(
    (item, pos, array) =>
      array
        .map((mapItem) => mapItem.details["header"])
        .indexOf(item.details["header"]) === pos
  );

const relatedQueryTags = (query: string) => {
  let tags: any = relatedTags.filter((tags) =>
    tags.find((tag) => tag.toLowerCase() === query.toLowerCase())
  );
  tags = removeDuplicates([].concat(...tags));
  return tags.filter((tag: TagType | SkillType) => tag !== query);
};

const relatedQueryMatches = (
  projects: IProject[],
  query: string
): IProjectSearch[] => {
