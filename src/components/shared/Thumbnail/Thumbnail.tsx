...
export const Thumbnail = (props: IThumbnailProps): JSX.Element => {
  ...
  return (
    <Container
      ...
      aria-hidden={!data.thumbnail}
    >
      <LinkStyle
        onClick={props.thumbnailClick}
        style={{ flex: !data.thumbnail ? 1 : "inherit" }}
      >
        <LinkWrapper
          link={link}
          isExternal={Boolean(data.file)}
          tabIndex={!data.thumbnail ? -1 : undefined}
        >
          <ImageWrapper>
      ...
      {props.showFull &&
        ...
        data.highlights.map((highlight, index) => (
          <Details key={index}>
            ...
            {highlight.tags && !hideTags && (
              <Tags tags={highlight.tags} setQuery={props.setQuery} />
            )}
          </Details>
        ))}
    </Container>
